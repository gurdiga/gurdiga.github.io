...
First, in my [`.bashrc`][1] I am exploiting the `PROMPT_COMMAND` environment variable. This variable is useful because Bash executes its content after executing any command that I type. For example: if I type the `date` command, and hit Return, Bash will execute the `date` command, and after that will execute the commands in the `PROMPT_COMMAND` environment variable. Here is a quick example:
...
The fact that it executes on every command doesn’t seem right, so I look at which command was executed, and only execute the function when I execute a `cd` command[^2].

Now, I only need to create the `.bash_history` in a directory, and when I `cd` into it, Bash picks it up. And yes, I also have a function that creates the `.bash_history` file: [`inith`][0]. 🙂
...
- call `initv` which does a similar thing for Vim;
- cd home and back so that Bash picks up the local `.bash_history`.
...
[1]: https://github.com/gurdiga/dotfiles/blob/80bd639/.bashrc.my
